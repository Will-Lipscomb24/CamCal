...
traj_time = 30       # seconds (total duration of trajectory)
...
bot_start = np.array([1, -1, 0])
bot_end = np.array([0, 0, 0])

# interpolation parameter
alpha = np.linspace(0, 1, N)
print(alpha.shape)
px = bot_start[0] * (1 - alpha) + bot_end[0] * alpha
py = bot_start[1] * (1 - alpha) + bot_end[1] * alpha
pz = bot_start[2] * (1 - alpha) + bot_end[2] * alpha
...
file_path = os.path.join(output_dir, f'trajectory_{hz:.2f}hz_line.csv' )
